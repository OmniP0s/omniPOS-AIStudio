export function validateZatcaCompliance(invoiceHash: unknown, qrBase64: unknown, ublXml: unknown, isB2B: unknown) {
  ...
  const qrValue = String(qrBase64);
  const xmlValue = typeof ublXml === "string" ? ublXml : "";
  const passesXsd = Boolean(xmlValue && xmlValue.includes("urn:oasis:names:specification:ubl:schema:xsd:Invoice-2"));
  return {
    statusCode: 200,
    body: {
